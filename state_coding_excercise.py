...
class CombinationLock:
    def __init__(self, combination):
        self.combination = [str(element) for element in combination]
        self.status = LockStates.LOCKED.name
        self.entry = []
    ...
    def enter_digit(self, digit):
        # todo
        self.entry.append(str(digit))
        if len(self.entry) == len(self.combination):
            if self.entry == self.combination:
                self.status = LockStates.OPEN.name
            else:
                self.status = LockStates.ERROR.name
        elif len(self.entry) < len(self.combination):
            self.status = ''.join(self.entry)
        else:
            self.reset()
...
class FirstTestSuite(unittest.TestCase):
    def test_success(self):
        # inital status will always be locked
        cl = CombinationLock([1, 2, 3, 4, 5])
        self.assertEqual('LOCKED', cl.status)
